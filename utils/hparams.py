...
import os
import yaml
from option import args as totalargs
...
def set_hparams(config='', exp_name='', hparams_str='', print_hparams=True, global_hparams=True):
    if config == '' and exp_name == '':
        ...
        parser.add_argument('--reset', action='store_true', help='reset hparams',default=True)
        parser.add_argument('--debug', action='store_true', help='debug')
        args, unknown = parser.parse_known_args()
        print("| Unknow hparams: ", unknown)
    else:
        args = Args(config=config, exp_name=exp_name, hparams=hparams_str,
                    infer=False, validate=False, reset=True, debug=False)
    global hparams
    ...
    hparams_['sr_l1_loss'] =False
    hparams_['use_rrdb'] =totalargs.use_rrdb
    hparams_['lr_encoder'] =totalargs.lr_encoder
    hparams_['diffusion_net'] =totalargs.diffusion_net
    hparams_['sr_scale'] =totalargs.scale
    hparams_['patch_size'] =totalargs.patch_size
    hparams_['lr'] =totalargs.lr
    hparams_['diff_type'] =totalargs.diff_type
