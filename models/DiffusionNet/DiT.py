...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.cond_embedder = PatchEmbed(input_size, patch_size, 64, hidden_size)
        ...
        if(hparams['sr_scale'] ==4):
            self.cond_proj = nn.ConvTranspose2d(cond_dim * ((hparams['rrdb_num_block'] + 1) // 3),
                                                3, hparams['sr_scale'] * 2, hparams['sr_scale'],   #sr_scale 4
                                                hparams['sr_scale'] // 2) #反卷积
        else:
            self.cond_proj = nn.ConvTranspose2d(cond_dim * ((hparams['rrdb_num_block'] + 1) // 3),
                                                3, 7, hparams['sr_scale'],
                                                2) #反卷积
    ...
    def forward(self, x, t, cond, img_lr_up, noise):
        """
        Forward pass of DiT.
