...
import numpy as np
#from .Unetformer import DiTBlock
...
class UnetDual(nn.Module):
    def __init__(self, dim, out_dim=None, dim_mults=(1, 2, 4, 8), cond_dim=32):
        ...
        encoder_channels = (64, 128, 192, 256)
        decode_channels=64
        self.decoder = Decoder(dim, encoder_channels, decode_channels, dropout=0.1, window_size=2, out_channel=64)
        #self.w = torch.nn.Parameter(torch.FloatTensor([0.5]), requires_grad=True)
    ...
    def forward(self, x, time, cond, img_lr_up, noise=None):
        ...
        #trans
        x2 = self.decoder(t, res2[0], res2[1], res2[2], x1)
        attention_weight=0.25 #fuse with determined value
        x1=self.final_conv(x)
        x=(1-attention_weight)*x+attention_weight*x2
