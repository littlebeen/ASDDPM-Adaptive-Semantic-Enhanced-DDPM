...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, img_hr, img_lr, img_lr_up, t=None, *args, **kwargs):
        ...
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()  \
            if t is None else torch.LongTensor([t]).repeat(b).to(device)  #生成0-timesteps之间的随机数,每张图不一样，生成了16个，因为每个batch16张图
        if hparams['use_rrdb']:
            if hparams['fix_rrdb']:
                self.rrdb.eval()
                with torch.no_grad():
        ...
        p_losses, piexl_loss, x_tp1, noise_pred, x_t, x_t_gt, x_0 = self.p_losses(x, t, cond, img_lr_up, *args, **kwargs)
        ret = {'q': p_losses}
        if piexl_loss != 0 :
            ret['piexl'] = piexl_loss
        if hparams['use_rrdb'] and not hparams['fix_rrdb']:
    ...
    def p_losses(self, x_start, t, cond, img_lr_up, noise=None):
        ...
        noise_pred = self.denoise_fn(x_tp1_gt, t, cond, img_lr_up,noise)
        ...
        piexl_loss=0
        if(hparams['sr_l1_loss']):
            x_0=(
                extract(self.sqrt_recip_alphas_cumprod, t, x_tp1_gt.shape) * x_tp1_gt -
                extract(self.sqrt_recipm1_alphas_cumprod, t, x_tp1_gt.shape) * noise_pred
            )   
            x_0.clamp_(-1.0, 1.0)
            piexl_loss=F.l1_loss(x_0, x_start)
        loss+=piexl_loss * 0.5
        loss.backward()
        return loss,piexl_loss, x_tp1_gt, noise_pred, x_t_pred, x_t_gt, x0_pred
    ...
    def p_sample(self, x, t, cond, img_lr_up, noise_pred=None, clip_denoised=True, repeat_noise=False):
        if noise_pred is None:
            noise_pred = self.denoise_fn(x, t, cond=cond, img_lr_up=img_lr_up,noise=1)
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance, x0_pred = self.p_mean_variance(
            x=x, t=t, noise_pred=noise_pred, clip_denoised=clip_denoised)
        #save_image(self.res2img(x0_pred, img_lr_up),'./image/test'+str(t)+'.png')
    ...
    def sample(self, img_lr, img_lr_up, shape, save_intermediate=False):
        ...
        if not hparams['res']:
            ...
            img = torch.randn(shape, device=device)  #进这里
        if hparams['use_rrdb']:
            rrdb_out, cond = self.rrdb(img_lr, True)
        else:
            rrdb_out = img_lr_up
        ...
        #save_image(img,'./image/final.png')
        if save_intermediate:
    ...
    def img2res(self, x, img_lr_up, clip_input=None):
        if clip_input is None:
            clip_input = hparams['clip_input']   #true
        if hparams['res']:  #true
